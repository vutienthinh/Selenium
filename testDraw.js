...
describe('Yes Option Selected', function () {
  ...
  it('03 User Can Not Draw', async function () {
    ...
    await driver.findElement(By.className('lower-canvas')).then(async function (elm) {
      // console.log(elm);
      // await actions.click(elm)
      //   .move({ x: 10, y: 50, origin: Origin.VIEWPORT })
      //   .move({ x: 50, y: 10, origin: Origin.VIEWPORT })
      //   .move({ x: 100, y: 150, origin: Origin.VIEWPORT })
      //   .move({ x: 150, y: 100, origin: Origin.VIEWPORT })
      //   .release()
      //   .perform();
      // await actions.move({ x: 30, y: 80, origin: elm })
      //   .click()
      //   .move({ x: 50, y: 100, origin: Origin.VIEWPORT })
      //   .release()
      //   .perform();
      await actions.dragAndDrop(elm , {x: 80, y: 40} ).perform();
      // const imgStr = await driver.executeScript('return document.querySelector(".lower-canvas").toDataURL();')
